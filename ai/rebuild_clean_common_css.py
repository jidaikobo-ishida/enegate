...
print("--- Rebuild Clean common.css (Fixing Shift-JIS & Comments) Start ---")
...
# 3225行目の #list #article, 部分の削除
# (テスト環境と本番環境で一部ズレがある可能性があるため、安全な文字列置換)
...
# --- 変更3: スマホ用メディアクエリ（max-width: 736px）の末尾に、これまでの正常なスマホ調整CSSを一括追記 ---
# スマホ用メディアクエリの終了部分を探して、その直前に追記する。
# 終了部分のマーカーとして、最後のスタイル規則を利用。
# クリーンな状態でのスマホ用メディアクエリの末尾は、
# #iteminfo .scrolltb::-webkit-scrollbar-thumb{ background: #999; border-radius: 5px;} } 
# もしくはそれに近い部分。
sp_end_marker = """	#iteminfo .scrolltb::-webkit-scrollbar-thumb{ background: #999; border-radius: 5px;}
}"""

sp_end_replacement = """	#iteminfo .scrolltb::-webkit-scrollbar-thumb{ background: #999; border-radius: 5px;}
...
if sp_end_marker_lf in content_lf:
    ...
    print("Successfully rebuilt and appended responsive styles to common.css.")
else:
    # 別のマーカーでの検索（最後の } 直前）
    print("Marker not found, attempting broader end scan.")
    # 最後の } の直前に挿入するフォールバック
    idx_last_brace = content_lf.rfind("}")
    if idx_last_brace != -1:
        # 最後の } の前にインサート
        # (ただし、最後のメディアクエリ全体の整合性に注意)
        pass
